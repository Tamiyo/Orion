...
public:
  ...
  static SyntaxNode createRoot(GreenNode node) {
    return SyntaxNode(syntax::detail::SyntaxNode::createRoot(std::move(node)));
  }

  /// \brief Wrap an existing untyped core SyntaxNode.
  ///
  /// \param raw The core SyntaxNode to wrap.
  explicit SyntaxNode(syntax::detail::SyntaxNode raw) : raw(std::move(raw)) {}
...
public:
  ...
  explicit SyntaxToken(syntax::detail::SyntaxToken raw) : raw(std::move(raw)) {}
...
public:
  ...
  explicit SyntaxElement(syntax::detail::SyntaxElement core)
  ...
  ///
  /// Implicit so callers can write `SyntaxElement e = node;`, matching the
  /// implicit converting behaviour of `std::variant`.
  ///
  /// \param node The SyntaxNode<Kind> to lift into a SyntaxElement.
  SyntaxElement(SyntaxNode<Kind> node) // NOLINT(*-explicit-constructor)
      : std::variant<SyntaxNode<Kind>, SyntaxToken<Kind>>(node),
        raw(syntax::detail::SyntaxElement(node.raw)) {}

  /// \brief Build a SyntaxElement from a typed SyntaxToken.
  ///
  /// Implicit; see the SyntaxNode overload for rationale.
  ///
  /// \param token The SyntaxToken<Kind> to lift into a SyntaxElement.
  SyntaxElement(SyntaxToken<Kind> token) // NOLINT(*-explicit-constructor)
      : std::variant<SyntaxNode<Kind>, SyntaxToken<Kind>>(token),
        raw(syntax::detail::SyntaxElement(token.raw)) {}
...
private:
  syntax::detail::SyntaxElement raw;
};
...
private:
  syntax::detail::SyntaxIterator raw;
};
...
private:
  syntax::detail::SyntaxChildren raw;
};

/// \brief Forward range over a SyntaxNode<Kind>'s child elements (nodes and
/// tokens). Wraps syntax::detail::SyntaxChildrenWithTokens.
