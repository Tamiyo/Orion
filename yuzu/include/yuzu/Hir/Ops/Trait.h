...
///
/// A trait's result is a `const Type *`, where null means Self (the
...
public:
  ...
  void registerTrait(std::u32string_view trait, const Type *result) {
  ...
  void registerImpl(std::u32string_view trait, const Type *operand) {
  ...
  [[nodiscard]] std::optional<const Type *>
  lookupImpl(std::u32string_view trait, const Type *operand) const {
...
private:
  ...
  llvm::DenseMap<std::u32string_view, const Type *> traits;
  llvm::DenseMap<std::pair<std::u32string_view, const Type *>, const Type *>
      impls;
};
