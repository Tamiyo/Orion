...
/// Emit each Variant's section: a `<V>_FIRST` sentinel, the variant
/// itself, every concrete Node under it (sorted by source order), and
/// a `<V>_LAST` sentinel. The sentinels are what makes range-check
/// `classof` implementations on `Variant` cheap.
