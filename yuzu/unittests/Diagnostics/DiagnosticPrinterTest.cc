...
TEST_F(DiagnosticPrinterTest, RendersSimpleErrorWithCaret) {
  ...
  diagnostics.error(span(src, 2, 3), "expected expression").emit();
...
1 | 2 ^ 2
  |   ^
            ...
            printer.printToString(diagnostics.getDiagnostics()[0]));
}

TEST_F(DiagnosticPrinterTest, IncludesCodeInHeader) {
  const SourceId src = sources.add("<repl>", U"2 ^ 2");
  diagnostics.error(span(src, 2, 3), "expected expression")
      .code("E0001")
      .emit();
...
TEST_F(DiagnosticPrinterTest, RendersSecondaryLabelOnSameLine) {
  ...
  diagnostics.error(span(src, 4, 5), "expected expression")
...
TEST_F(DiagnosticPrinterTest, RendersMultipleSecondariesRustcStyle) {
  // Two secondaries on the same line with messages stack rustc-style:
  // a connector row of `|`s at each label's start column, then per-label
  // rows rightmost-first; labels still pending (to the left) keep their
  // `|` connector until rendered.
  const SourceId src = sources.add("<source>", U"2.1 + 2");
  diagnostics.error(span(src, 0, 7), "binary operands have incompatible types")
      .label(span(src, 0, 3), "left operand has type `float`")
      .label(span(src, 6, 7), "right operand has type `int`")
      .emit();

  const DiagnosticPrinter printer(sources);
  EXPECT_EQ(R"(error: binary operands have incompatible types
 --> <source>:1:1
  |
1 | 2.1 + 2
  | ^^^^^^^
...
TEST_F(DiagnosticPrinterTest, ReportsLineColForLaterLine) {
  ...
  diagnostics.error(span(src, 8, 9), "look here").emit();
...
TEST_F(DiagnosticPrinterTest, SeverityWordTracksDiagnosticKind) {
  ...
  diagnostics.warning(span(src, 0, 1), "unused").emit();
