...
TEST(DiagnosticsEngineTest, StartsEmpty) {
  ...
  EXPECT_EQ(0u, diagnostics.getErrorCount());
  EXPECT_EQ(0u, diagnostics.getWarningCount());
  EXPECT_FALSE(diagnostics.hasErrors());
}

TEST(DiagnosticsEngineTest, ErrorEmitsWithPrimaryLabel) {
  DiagnosticsEngine diagnostics;
  diagnostics.error(span(0, 1), "expected expression").emit();
  ...
  // The diagnostics seeds the builder with one Primary label spanning the
...
TEST(DiagnosticsEngineTest, BuilderChainsCodeLabelsAndNotes) {
  DiagnosticsEngine diagnostics;
  diagnostics.error(span(2, 3), "expected expression")
      .code("E0001")
...
TEST(DiagnosticsEngineTest, CountsTrackSeverity) {
  DiagnosticsEngine diagnostics;
  diagnostics.error(span(0, 1), "first").emit();
  diagnostics.warning(span(1, 2), "second").emit();
  diagnostics.error(span(2, 3), "third").emit();
  diagnostics.remark(span(3, 4), "fourth").emit();

  EXPECT_EQ(4u, diagnostics.getDiagnostics().size());
  EXPECT_EQ(2u, diagnostics.getErrorCount());
  EXPECT_EQ(1u, diagnostics.getWarningCount());
...
TEST(DiagnosticsEngineTest, BuilderWithoutEmitDropsDiagnostic) {
  ...
  (void)diagnostics.error(span(0, 1), "nevermind");
...
TEST(DiagnosticsEngineTest, PrimaryLabelAddsAdditionalPrimary) {
  // The first call to diagnostics.error() seeds one Primary label. Callers
  // can promote a second span to Primary status with `.primaryLabel`.
  DiagnosticsEngine diagnostics;
  diagnostics.error(span(0, 1), "msg")
