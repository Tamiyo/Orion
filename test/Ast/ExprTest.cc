...
TEST(ExprTest, CastWithChildren) {
  ...
  const std::optional<BinaryExpr> AstAsBinary = Ast->tryCast<BinaryExpr>();
  ...
  const std::optional<LiteralExpr> LhsAsLiteral = Lhs->tryCast<LiteralExpr>();
  const std::optional<LiteralExpr> RhsAsLiteral = Rhs->tryCast<LiteralExpr>();
